...
- **checkpoint_identity_deliveries** — local-only record that an adapter delivered the opaque host identity `(host, session_id, root_turn_id)` to the root agent. The Cursor stop verifier uses it to enqueue at most one follow-up when that identity was delivered and the Terminal Memory commit is still missing. The table stores only opaque identity and a timestamp, has no FTS or sync triggers, and is excluded from Memory search, context, counts, JSON/project/chunk exports, pending mutations, cloud materialization, and Obsidian output. It is not a checkpoint and never captures prompts.
...
outcome, verifies the pinned four-axis Compatibility tuple, reconstructs its
exact Git commit/tree membership from embedded object bodies, and checks that
the supplied source artifacts are the blobs committed at that revision. The
self-contained proof works in shallow or extracted source without network or
repository mutation. The result reports post-install readiness as `not_verified`;
...
| `ENGRAM_CODEX_RECALL_CANARY`    | Opt-in Codex lifecycle treatment: `targeted-recall` is cue-only for every SessionStart source; `targeted-recall-exact-session` may add bounded exact-session context only after compact. Unknown values do not enable a canary. | (unset — existing broad treatment) |
