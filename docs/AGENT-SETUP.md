...
Writes use atomic replacement. Fresh setup and supported legacy upgrades converge on one canonical detailed skill, one short cue, one synchronous Stop verifier, and thin CLI/MCP adapters without legacy protocol copies. A successful rerun is byte-stable, does not create duplicate MCP blocks, and can recover after an interrupted first run.
