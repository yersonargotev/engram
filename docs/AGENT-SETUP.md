...
This installs the portable Agent Plugin into Cursor's local plugin directory at `~/.cursor/plugins/local/engram`: `plugin.json`, editorial `skills/engram-memory`, and a copy of the running binary. The portable package includes a plugin `mcp.json`; setup replaces it with the verified user-level registration when Cursor CLI is installed, so editor and CLI share one Engram server identity. Stable setup pins the plugin to the binary's release version and commit. A second run recopies the running binary and refreshes only the Engram-owned plugin, leaving other local plugins alone.

Setup also writes user-level Cursor hooks in `~/.cursor/hooks.json`. `sessionStart` injects the short activation cue from the installed skill. `beforeSubmitPrompt` forwards Cursor's opaque `conversation_id` and `generation_id` as the checkpoint identity `(host=cursor, session_id, root_turn_id)` on every actual user prompt, and records that delivery only after the local ledger write succeeds. Changing cwd or project does not change that identity; invalid events, ledger failures, and the recovery follow-up prompt emit no new identity. `stop` may emit one follow-up only when that root turn received the opaque identity and still has no Memory checkpoint. If identity was never delivered, no user-looking finalize prompt is enqueued and the incomplete checkpoint remains visible. The follow-up is not an independent root user turn and does not create a second checkpoint identity. The hook commands parse Cursor JSON, call Engram, and return; they do not decide durability. Prompt and subagent capture stay off. A second run refreshes Engram-owned hook entries and leaves user-owned hooks in place.

Setup also writes an Engram-owned global `~/.cursor/mcp.json` registration pointing to the installed plugin binary. Cursor CLI does not discover the local plugin's `mcp.json` on the tested installation. Setup verifies the global registration through `agent mcp list` before replacing an older owned entry or removing the plugin MCP declaration; a failed check restores the original file. Existing customized Engram entries, including entries with extra fields or custom executable paths, and unrelated MCP servers are preserved. The CLI may report `needs approval`; approve Engram through Cursor's normal MCP approval flow, then check `agent mcp list-tools engram`. Setup never approves a server for you and never writes project-level `.cursor` files. If the CLI is unavailable or a customized native entry is preserved, setup retains the plugin MCP declaration. A project `.cursor/mcp.json` entry can take precedence over the global one; setup reports a conflict when that entry points elsewhere.
...
The command does not install, repair, rewrite user configuration, or read captured content. It reports `plugin`, `skill`, isolated `mcp` protocol capability, `hooks`, `cli_mcp` host discovery and approval, and `cli_use` separately. The `plugin` check compares the copied MCP binary with the running Engram executable as well as the pinned version and commit. Its `binary_state` evidence distinguishes `missing`, `stale`, and `current`; `plugin_binary_stale` means setup must recopy the binary even if the pinned release file still matches. Ready hooks include `session_start`, `prompt_submit`, and `stop` evidence. For isolated MCP capability, status starts the configured command against a temporary Engram data directory, disables autosync and baseline collection for that child, and requires a bounded `initialize` plus `tools/list` round trip with exactly the five agent-profile tools. `cli_mcp` runs Cursor's own inventory and, after approval, checks its five-tool catalog from a disposable copy of the Cursor configuration; `pending` means Cursor discovered Engram but awaits approval. `cli_use` distinguishes no observed call from a successful Cursor MCP call in the content-free baseline. It does not claim that an earlier Cursor call came from CLI rather than the editor; a fresh CLI session must be verified separately. A missing CLI is `unavailable` and does not change editor plugin readiness. `user_rules` stays `unknown` because setup cannot inspect the Settings store. A plugin-only installation is `partial`, never `checkpoint_ready`. Leftover user skill copies, including Packy `engram-memory-cli`, are reported and are not treated as the canonical plugin skill.

Cursor status proves CLI discovery and tool availability only for the inspected CLI process. It cannot prove that an already-running editor has reloaded or admitted the local plugin. The default-on content-free baseline distinguishes server process, initialization, tool discovery, and actual calls after Cursor starts it.

**Manual** — copy `plugin/engram/` to `~/.cursor/plugins/local/engram`, place the Engram binary at `bin/engram`, register that absolute binary path in `~/.cursor/mcp.json` with `mcp --tools=agent`, remove the plugin copy of `mcp.json` after `agent mcp list` discovers the native entry, and add user-level `sessionStart` / `beforeSubmitPrompt` / `stop` hook entries in `~/.cursor/hooks.json` that call that binary (`lifecycle session-start --host=cursor`, `lifecycle prompt-submit --host=cursor`, and `checkpoint verify-stop --host=cursor`). Then restart Cursor or run Developer: Reload Window.
...
`sessionStart` re-delivers this short activation cue from the installed `engram-memory` skill after compaction. `beforeSubmitPrompt` delivers the opaque checkpoint identity for that root user turn. Do not paste the full Memory rubric into User Rules or a global `.mdc` file.
