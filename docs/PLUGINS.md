...
| Cursor | `engram setup cursor` installs that portable package into `~/.cursor/plugins/local/engram` and writes a separate user-level `~/.cursor/hooks.json` adapter for the activation cue and stop follow-up. |
