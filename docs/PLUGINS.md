...
|---|---|
| Agent Plugin (portable) | Agent Plugins 1.0 package under `plugin/engram/`: root `plugin.json`, editorial `engram-memory` skill, and `mcp.json` for the agent tool profile with a plugin-relative `./bin/engram` command. No hooks or rules. |
| Cursor | `engram setup cursor` installs that portable package into `~/.cursor/plugins/local/engram`, rewrites the installed MCP command to the copied binary's absolute path so Cursor does not resolve `./bin/engram` against the workspace cwd, and writes a separate user-level `~/.cursor/hooks.json` adapter for the activation cue, root-turn identity, and stop follow-up. |
