...
| `checkpoint_identity_deliveries` | Local-only proof that a host adapter delivered opaque root-turn identity to the agent. Cursor's stop verifier follows up only when this row exists and the checkpoint is still missing. | `internal/store/checkpoint.go`, `cmd/engram/lifecycle.go`, `cmd/engram/checkpoint.go` |
