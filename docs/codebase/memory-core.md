...
| `memory_checkpoints` / `memory_checkpoint_references` | Local-only root-turn dispositions and typed Memory references, excluded from every durable-Memory and replication surface. | `internal/store/checkpoint.go`, `internal/memoryops/checkpoint.go` |
