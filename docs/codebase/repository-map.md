...
| `internal/recallstudy` | Frozen contract and cohort verification, isolated consented execution, deterministic pairing, private row validation, aggregate-only reports, preregistered gate evaluation, and Codex-scoped disposition selection. | Open held-out task inputs before successful calibration, mutate product policy, enable rollout, or persist study rows in Memory/store/sync/cloud. |
