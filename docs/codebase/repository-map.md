...
| `internal/recallstudy` | Frozen contract and cohort verification, isolated consented execution, deterministic pairing, private row validation, aggregate-only reports, preregistered gate evaluation, Codex-scoped disposition selection, and read-only verification of the content-addressed distribution outcome at its exact source revision. | Open held-out task inputs before successful calibration, mutate product policy, enable rollout, contract installed state, claim post-install readiness, or persist study rows in Memory/store/sync/cloud. |
