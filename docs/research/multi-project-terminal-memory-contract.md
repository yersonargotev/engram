# Proposed multi-project Terminal Memory contract

Historical research record, 2026-09-09. Retained as supporting evidence for
[ADR-0012](../adr/0012-group-terminal-memory-by-project.md), which owns the
accepted design. Proposal wording below records the investigation at the time;
current runtime remains single-project until separately approved implementation.

Status: formal design proposal authorized by the maintainer for [issue #159](https://github.com/yersonargotev/engram/issues/159). Runtime behavior is not implemented; accepted architecture sources must be integrated through the design brief.

Authoritative published [Agent Brief and formal contract](https://github.com/yersonargotev/engram/issues/159#issuecomment-5613339506).

This proposal selects project-specific internal sessions beneath an unchanged host
conversation identity and one atomic checkpoint containing explicit project groups.
The maintainer authorized formalization with agent-operated Memory management as
a required product constraint. This authorizes the design direction, not an implicit
migration, release, or repair of existing sessions. Implementation and migration
must be separately scoped and approved as required by #159.
...
This explicitly revises ADR-0011's cardinality and the glossary's Mixed definition
when the design is accepted; the current definitions remain authoritative meanwhile.
...
Inspected maintained source at `8189fca0be53d84ce6c5b36706065416b21be08b`.
The current implementation still has one project per record and binds new inline
Memories to the host session ID. Relevant sources:

- [Core checkpoint operation](../../internal/memoryops/checkpoint.go), including replay and preflight.
- [Store checkpoint transaction and proposal schema](../../internal/store/checkpoint.go).
- [Project lifecycle and session persistence](../../internal/store/store.go).
- [CLI checkpoint adapter](../../cmd/engram/checkpoint.go) and [MCP adapter](../../internal/mcp/checkpoint.go).
- [Protocol range](../../internal/protocolcontract/contract.go).
- [ADR-0006](../adr/0006-own-memory-checkpoints-in-core.md), [ADR-0011](../adr/0011-preserve-mixed-terminal-memory-outcomes.md), and [prior runtime research](host-conversation-multiple-projects.md).

Verification during this proposal used a binary built from that revision and a
temporary `ENGRAM_DATA_DIR` with synthetic identities. Inline turn A returned
`created`; inline turn B in the same host session returned
`checkpoint_project_mismatch` with `cause: session_project_binding`. The fixture
directory was removed afterward. Targeted existing checkpoint tests passed in
Store, memoryops, CLI, and MCP:

```text
go test ./internal/store ./internal/memoryops ./internal/mcp ./cmd/engram -run 'Test.*(Checkpoint|RecordSaved)' -count=1
```

These checks verify the current baseline, not the unimplemented proposed model.
