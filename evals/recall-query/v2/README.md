# Concept versus identifier Recall v2
...
rules before evaluated execution. `source.json` additionally commits fixture generation, execution, reporting and CLI code; source changes or added evaluator files are rejected. `internal/recallquery` contains the compiled
...
`-verify` reads the contract, source commitments and calibration; it never opens held-out. The default executes only
...
fails visibly. Failed execution produces aggregate unavailable-quality evidence (`evidence_available=false`, with failed needs counted as unknown);
...
Spanish cases are omitted in v2 to isolate the original English observation;
...
The v1 exploratory publication remains archived. Review found incomplete code binding and ambiguous empty-evidence fields; v2 freezes the repaired evaluator and a newly authored held-out cohort. Calibration uses shorter same-intent concept anchors to exercise cumulative exposure. See [FINDINGS.md](FINDINGS.md) for v2 results.
