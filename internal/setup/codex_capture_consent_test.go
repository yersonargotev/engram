...
func TestInstallCodexRerunNeitherEnablesNorRevokesDiagnosticCaptureConsent(t *testing.T) {
	t.Run("fresh remains default off", func(t *testing.T) {
		...
		for _, contentType := range []string{store.CaptureContentTypePrompt, store.CaptureContentTypeSubagentOutput} {
			status, err := store.InspectCaptureConsentReadOnly(dataDir, "engram", contentType, "", time.Now().UTC())
			if err != nil {
				t.Fatalf("inspect fresh %s consent: %v", contentType, err)
			}
			if status.SchemaPresent || status.Consent != nil {
				t.Fatalf("fresh setup created %s capture consent state: %#v", contentType, status)
			}
		}
	...
	for _, contentType := range []string{store.CaptureContentTypePrompt, store.CaptureContentTypeSubagentOutput} {
		t.Run("explicit "+contentType+" project grant survives rerun", func(t *testing.T) {
			resetSetupSeams(t)
			home := useTestHome(t)
			dataDir := filepath.Join(home, ".engram")
			...
			status, err := store.InspectCaptureConsentReadOnly(dataDir, "engram", contentType, "", now)
			if err != nil {
