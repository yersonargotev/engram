...
func inspectManagedPackProtocolDeclaration(skillChecks []CodexIntegrationCheck) protocolcontract.Declaration {
	...
	for _, check := range skillChecks {
		...
		declaration := protocolcontract.Declaration{
			...
			Provenance: fmt.Sprintf("repository:https://github.com/yersonargotev/engram.git;manifest:%s#sha256:%s", filepath.Clean(manifestPath), hex.EncodeToString(manifestDigest[:])),
		}
		skillHash := codexStatusEvidenceValue(check, "sha256")
		if manifest.Version == legacyManagedPackVersion && skillHash == legacyManagedPackSHA256 {
		...
		if skillHash != fixture.Distributions.ManagedPack.SkillSHA256 {
			declaration.Provenance = ""
			candidates = append(candidates, declaration)
			continue
		}
