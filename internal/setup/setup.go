...
For every settled root user turn, finalize exactly one Terminal Memory commit through mem_checkpoint: saved, needs_review, or skipped(no_durable_knowledge). Apply the canonical checkpoint rubric once, after all causal agent, tool, subagent, compaction, and automatic-continuation work has settled.

Keep the supplied host, session ID, and root-turn ID as opaque identity. Reuse that exact identity across continuations. Tool calls, subagents, compaction events, and verifier continuations do not create independent checkpoints, and only the root agent finalizes the checkpoint.

Choose the disposition with the canonical rubric:
- saved — the turn produced concise, safe, independently useful durable knowledge, such as a non-obvious verified fix, a decision with its reason, a reusable invariant or workflow, or a durable constraint. Prefer inline Memories so creation and checkpointing are atomic; reuse an exact existing Memory when it already contains the result.
- needs_review — potentially durable knowledge is ambiguous, incomplete, conflicting, or sensitive. Retain one bounded, redacted proposal explaining what needs review; this is not a fallback for a tool or persistence failure.
- skipped(no_durable_knowledge) — neither the saved nor review rubric applies. Routine work already represented by maintained source, a simple explanation, or status-only activity normally belongs here. Integration failures are not skips.

Treat created and an exact already_recorded replay as success. Surface conflicts or persistence failures; never invent a replacement identity or overwrite a terminal result.

### SPECIALIZED WORKFLOWS

Session summaries and independent Memory saves are optional curation operations, not default completion requirements. Select the curation profile explicitly for those workflows, including a long-running handoff with material loss risk. Host activity, prompt save, and passive Content capture belong to the lifecycle profile and remain subject to explicit capture policy. Destructive and operational maintenance belongs to the admin profile.

### AFTER COMPACTION

Recover only the context needed to continue the original root user turn. Keep its opaque checkpoint identity and finalize the same turn after the remaining causal work settles. Compaction is not an independent turn and does not itself require a Session summary, independent save, broad project Recall, or checkpoint.
`
...
Summarize:
- Goal
- Current user instructions and constraints
- Key technical discoveries and decisions
- Completed work
- Remaining next steps
- Relevant files changed
- The opaque Engram checkpoint identity, if one was supplied

Keep it concise and high-signal. Do not invent missing identity or evidence.

End with this policy cue: Continue the original root user turn, recover only the context needed for that work, reuse its exact opaque checkpoint identity, and finalize one saved, needs_review, or skipped(no_durable_knowledge) Terminal Memory commit only after all causal work settles. Compaction is not an independent turn and does not require a Session summary or independent save.`
...
func installCodexWithOptions(options InstallOptions) (*Result, error) {
	...
	if mcpRegistrationPreserved {
		result.Preserved = appendUnique(result.Preserved, "mcp_servers.engram")
	}
...
func captureCodexSetupFiles() map[string]setupFileSnapshot {
	paths := []string{
		...
		codexLegacyRetirementPath(codexInstructionsPath()),
		codexCompactPromptPath(),
		codexLegacyRetirementPath(codexCompactPromptPath()),
	}
...
func retireCodexLegacyActivation(configPath string) ([]string, error) {
	...
	for _, setting := range codexLegacySettings() {
		if blocked[setting.key] {
			continue
		}
		value, found, valid := topLevelTOMLString(content, setting.key)
		generatedKnown, readErr, statErr := codexLegacyFileOwned(setting.path, setting.content)
	...
	staged := make([]codexLegacySetting, 0, len(owned))
	...
	for _, setting := range owned {
		if known, _, _ := codexLegacyFileOwned(setting.path, setting.content); !known {
			if restoreErr := restoreStaged(); restoreErr != nil {
				return preserved, fmt.Errorf("retire Codex legacy activation %s: ownership changed; rollback failed: %v", setting.key, restoreErr)
			}
		...
		if err := renameFileFn(setting.path, stagePath); err != nil {
			...
			return preserved, fmt.Errorf("stage Codex legacy activation %s: %w", setting.key, err)
		}
		staged = append(staged, setting)
		if known, _, _ := codexLegacyFileOwned(stagePath, setting.content); !known {
			if restoreErr := restoreStaged(); restoreErr != nil {
	...
	current, readErr := readFileFn(configPath)
	if readErr != nil || string(current) != content {
		if restoreErr := restoreStaged(); restoreErr != nil {
			return preserved, fmt.Errorf("retire Codex legacy activation: config ownership changed; rollback failed: %v", restoreErr)
		}
		return preserved, fmt.Errorf("retire Codex legacy activation: config ownership changed before publish")
	}
	for _, setting := range staged {
		if known, _, _ := codexLegacyFileOwned(codexLegacyRetirementPath(setting.path), setting.content); !known {
			if restoreErr := restoreStaged(); restoreErr != nil {
				return preserved, fmt.Errorf("retire Codex legacy activation %s: staged content changed; rollback failed: %v", setting.key, restoreErr)
			}
	...
	if err := atomicWriteFileFn(configPath, []byte(updated), 0644); err != nil {
		if restoreErr := restoreStaged(); restoreErr != nil {
			return preserved, fmt.Errorf("retire Codex legacy activation config: %w; rollback failed: %v", err, restoreErr)
		}
...
func codexLegacyFileOwned(path, content string) (known bool, readErr, statErr error) {
	...
	return readErr == nil && statErr == nil && info.Mode().IsRegular() && string(data) == content, readErr, statErr
}
...
func codexMCPRegistrationPreserved(configPath string) bool {
	data, err := readFileFn(configPath)
	if err != nil {
		return false
	}
