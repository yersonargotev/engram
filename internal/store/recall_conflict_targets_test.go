...
func TestRecallEligibleRelationTargetsContextEnforcesAuthorityAndLifecycle(t *testing.T) {
	s := newTestStore(t)
	...
	targets, err := s.RecallEligibleRelationTargetsContext(context.Background(), []string{
	...
	empty, err := s.RecallEligibleRelationTargetsContext(context.Background(), nil, SearchOptions{})
	if err != nil || len(empty) != 0 {
		t.Fatalf("empty targets=%#v error=%v", empty, err)
	}
}

func TestRecallEligibleRelationTargetsContextPropagatesCancellationAndQueryFailure(t *testing.T) {
	s := newTestStore(t)
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if result, err := s.RecallEligibleRelationTargetsContext(ctx, []string{"sync-id"}, SearchOptions{}); !errors.Is(err, context.Canceled) || result != nil {
	...
	if result, err := s.RecallEligibleRelationTargetsContext(context.Background(), []string{"sync-id"}, SearchOptions{}); err == nil || result != nil {
